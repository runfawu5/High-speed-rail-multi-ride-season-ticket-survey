# High-speed-rail-multi-ride-season-ticket-survey

This repository hosts the questionnaire file of the questionnaire used in the study:

**“Joint optimization of seat allocation and pricing with multi-ride season tickets in high-speed railway services”**  
(Working paper)

## Contents
- `Questionnaire for multi-ride season ticket in HSR.pdf` - PDF 

## Questionnaire Structure
- **A. Screening Questions**
- **B. Travel Frequency**
- **C. Factors Affecting Your Travel Frequency**
- **D. Stated Preference Choice Scenarios**

## How to Cite
If you use or adapt this questionnaire, please cite:
- the associated paper (preferred), and
- this repository.

## License
This questionnaire is licensed under **CC BY 4.0**.  
You are free to share and adapt the material, provided that appropriate credit is given.

## Contact
For questions or collaboration, please contact: <runfawu5@gmail.com> or <wei.w.liu@polyu.edu.hk>.
